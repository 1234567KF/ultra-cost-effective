#!/usr/bin/env pwsh
# Seiko 极致节能 — Windows PowerShell 安装脚本
# 一条命令完成安装和预检
...
$SEIKO_ROOT = Split-Path -Parent $MyInvocation.MyCommand.Path

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "   Seiko · 极致节能 — Windows 安装脚本" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if ($SkipHeadroom) {
    ...
    Write-Host "    如需启用: pip install headroom-ai[all] && set SEIKO_HEADROOM=1" -ForegroundColor Gray
}
...
$presetFile = Join-Path $SEIKO_ROOT "presets" "$Preset.json"
if (Test-Path $presetFile) {
    ...
    # 设置环境变量
    $env:SEIKO_PLATFORM = $platform
    $env:SEIKO_PRESET = $Preset

    if ($presetConfig.layers.L1_tokenforge) {
        $env:SEIKO_LEVEL = $presetConfig.layers.L1_tokenforge.level
    }
} else {
    Write-Host "  ⚠ 预设文件不存在: $presetFile，使用默认 standard" -ForegroundColor Yellow
    $env:SEIKO_LEVEL = "medium"
    $env:SEIKO_PRESET = "standard"
}
...
$validatorPath = Join-Path $SEIKO_ROOT "helpers" "prefix-validator.cjs"
...
Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "   Seiko 安装完成！" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "    2. Seiko 将自动激活（always-on: true）" -ForegroundColor White
Write-Host "    3. 说出「token report」查看节省效果" -ForegroundColor White
Write-Host ""
Write-Host "  手动控制:" -ForegroundColor Yellow
Write-Host "    set SEIKO_OFF=1          # 临时关闭" -ForegroundColor White
