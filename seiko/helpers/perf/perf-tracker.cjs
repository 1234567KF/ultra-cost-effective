#!/usr/bin/env node
/**
 * perf-tracker.cjs — 全链路 Token 性能追踪器 (Seiko L0)
 *
...
function createSession() {
  return {
    sessionId: `seiko_${Date.now().toString(36)}`,
    startTime: Date.now(),
    totalCalls: 0,
    // Token 统计
    totalInputTokens: 0,
...
function watchMode() {
  console.log('🔍 Seiko Perf Tracker — Watch 模式 (Ctrl+C 退出)\n');
  let lastSize = 0;
...
function captureFromHook(raw) {
  ...
  // 估算 tokenforge 节省：根据 SEIKO_LEVEL 环境变量
  const level = process.env.SEIKO_LEVEL || 'medium';
  ...
  const seikoShouldApply = isBenefitCommandForTracker(base, command);

  const callData = {
    model: process.env.SEIKO_MODEL || 'deepseek-v4-flash',
    inputTokens: 0,
    ...
    // 跨技能生效标记
    effective: wasCompressed,  // true=Seiko生效, false=可能被绕过
    bypassed: seikoShouldApply ? !wasCompressed : false
  };
...
function main() {
  ...
  if (args.includes('--export')) {
    ...
    const filePath = args[idx + 1] || path.join(os.tmpdir(), 'seiko-calls.csv');
    const session = loadSession();
  ...
  if (args.includes('--audit') || args.includes('-a')) {
    ...
    console.log('  Seiko 生效审计');
    console.log('═'.repeat(50));
    console.log('');
    console.log(`总调用:       ${totalCalls}`);
    console.log(`Seiko 生效:   ${effectiveCalls}  (${totalCalls > 0 ? (effectiveCalls / totalCalls * 100).toFixed(1) : 0}%)`);
  ...
  if (session.totalCalls === 0) {
    console.log('═══════════════════════════════════');
    console.log('  Seiko Perf Tracker');
    console.log('═══════════════════════════════════');
    console.log('');
