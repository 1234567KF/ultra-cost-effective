#!/usr/bin/env node
/**
 * seiko-guard.cjs — 跨技能生效保障引擎 (Seiko 兼容层)
 *
 * 当项目中有多个技能共存时，确保 Seiko 的每一层优化确实生效：
 *   1. PreToolUse:  验证 Seiko 管道未被其他技能移除
 ...
 *
 * 用法:
 *   node seiko-guard.cjs pre-check "<command>"        # 执行前检查
 *   node seiko-guard.cjs post-verify <output_size>     # 执行后验证
 *   node seiko-guard.cjs audit                         # 本次会话生效审计
 *   node seiko-guard.cjs report                        # 跨技能兼容性报告
 */
...
const KNOWN_COMPRESSORS = [
  { pattern: /tokenforge/i,        name: 'tokenforge (Seiko)',         conflicts: false },
  { pattern: /headroom/i,          name: 'Headroom (Seiko CCR)',       conflicts: false },
...
function appendGuardEntry(entry) {
  ...
  log.totalChecks++;
  if (!entry.seikoApplied) log.bypassed++;
...
function preAgentSpawn(command = '', toolName = '') {
  ...
  try {
    ...
    return { isSpawn: true, hint: '[Seiko] Agent spawn 检测到，建议传递压缩上下文', contextHealth: null };
  }
}
...
/**
 * 检查命令中 Seiko 的注入是否仍然存在
 * @returns {{ safe: boolean, seikoPresent: boolean, warnings: string[], conflicts: string[] }}
 */
function preCheck(command) {
  if (!command) return { safe: true, seikoPresent: false, warnings: [], conflicts: [], reason: '空命令' };
  ...
  // 检测是否为 Seiko 受益命令（从 BENEFIT_COMMANDS 白名单判断）
  const base = (command.trim().split(/\s+/)[0] || '').toLowerCase().replace(/^.*[\\/]/, '');
  const seikoShouldApply = isBenefitCommand(base, command);
  ...
  for (const comp of KNOWN_COMPRESSORS) {
    if (comp.pattern.test(command)) {
      if (comp.conflicts && !comp.name.includes('Seiko')) {
  ...
  // 检查是否有 Seiko 被绕过的情况
  if (seikoShouldApply && !hasTokenforge && !hasHeadroom) {
    warnings.push(`命令应被 Seiko 压缩但未注入管道: ${command.slice(0, 80)}`);
  }
  ...
  // 如果其他技能已注入压缩，但 Seiko 也在
  if (conflicts.length > 0 && (hasTokenforge || hasHeadroom)) {
    warnings.push(`Seiko 与 ${conflicts.join(', ')} 可能产生双重压缩，建议禁用其一`);
  }

  return {
    safe: conflicts.length === 0,
    seikoPresent: hasTokenforge || hasHeadroom,
    engine: hasHeadroom ? 'headroom' : hasTokenforge ? 'tokenforge' : 'none',
    shouldApply: seikoShouldApply,
...
function postVerify(command, outputSize, stderr = '') {
  ...
  // 检测输出中是否有 Seiko 压缩摘要行（stderr 写到了输出）
  ...
  // 如果命令有 Seiko 管道但没检测到压缩标记
  if ((hadTokenforge || hadHeadroom) && !compressed) {
    hints.push(`⚠ 命令有 Seiko 管道但未检测到压缩标记，可能被绕过`);
  }
...
function auditEffectiveness() {
  ...
  const bypassed = report.bypassed || report.calls.filter(c => !c.seikoApplied).length;
  ...
  if (total > 0) {
    ...
    if (bypassed > 0) {
      ...
      const bypassedCalls = report.calls.filter(c => !c.seikoApplied).slice(-5);
...
function generateGuardStatement() {
  ...
  return `[Seiko生效保障: ${total}次调用, ${tfCalls}次tokenforge压缩, ${record.bypassed || 0}次绕过。勿禁用Seiko Hook。]`;
}
...
function main() {
  ...
  if (cmd === 'pre-check') {
    ...
    appendGuardEntry({
      phase: 'pre',
      command: command.slice(0, 120),
      seikoApplied: result.seikoPresent,
  ...
  if (cmd === 'post-verify') {
    ...
    const command = args.slice(2).join(' ') || process.env.SEIKO_LAST_COMMAND || '';

    const result = postVerify(command, outputSize, stderr);
    appendGuardEntry({
      phase: 'post',
      command: command.slice(0, 120),
      seikoApplied: result.compressed,
      outputSize,
      hints: result.hints,
  ...
  const log = loadGuardLog();
  console.log('═══════════════════════════════════');
  console.log('  Seiko Guard — 跨技能保障');
  console.log('═══════════════════════════════════');
  console.log(`  检查总数: ${log.totalChecks || log.calls.length}`);
  console.log(`  绕过次数: ${log.bypassed || log.calls.filter(c => !c.seikoApplied).length}`);
  console.log('');
  console.log('命令:');
