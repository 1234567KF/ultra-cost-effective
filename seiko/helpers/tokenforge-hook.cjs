 ...
 *  4. 干运行模式（--seiko-dry-run）
 *  5. 环境变量控制（SEIKO_LEVEL, SEIKO_OFF）
 */
...
function getEnvLevel() {
  return process.env.SEIKO_LEVEL || null;
}

function isSeikoOff() {
  return process.env.SEIKO_OFF === '1' || process.env.SEIKO_OFF === 'true';
}
...
function injectTokenforge(command, config, platform = 'claude') {
  ...
  // 检测是否有 --seiko-dry-run 标志
  const dryRun = command.includes('--seiko-dry-run');
  let cleanCommand = command.replace(/\s*--seiko-dry-run\b/g, '');

  // 检测是否已有 tokenforge 或 headroom 管道（Seiko 自己的注入）
  ...
  for (const comp of thirdPartyCompressors) {
    if (comp.pattern.test(cleanCommand)) {
      if (process.env.SEIKO_DEBUG) {
        process.stderr.write(`[seiko] ⚠ 检测到第三方压缩器 "${comp.name}"，跳过 Seiko 注入避免双重压缩\n`);
      }
      try {
        const guard = require('./seiko-guard.cjs');
  ...
  try {
    ...
    if (useHeadroom && process.env.SEIKO_DEBUG) {
      process.stderr.write(`[seiko] compressor-selector → headroom: ${decision.reason}\n`);
      if (decision.sessionMemory) {
        process.stderr.write(`[seiko] session-memory: ${decision.sessionMemory.split('\n')[0]}\n`);
      }
    }
  } catch {
  ...
  if (useHeadroom) {
    ...
    if (process.env.SEIKO_DEBUG) {
      process.stderr.write(`[seiko] ${hrCommand}\n`);
    }
    return hrCommand;
  }
...
function claudeHookFormat(input) {
  ...
  try {
    ...
    if (!command || shouldSkip(command) || isSeikoOff()) {
    ...
    return JSON.stringify({
      ...
      hookSpecificOutput: {
        ...
        permissionDecisionReason: `seiko: 自动注入 tokenforge ${config.type}/${config.level} 压缩`,
        modifiedInput: {
          command: newCommand
        }
      }
...
function qoderHookFormat(input) {
  ...
  try {
    ...
    return JSON.stringify({
      ...
      reason: `seiko: tokenforge ${config.type}/${config.level}`
    });
  } catch (e) {
...
function cliTest() {
  ...
  console.log('\n═══ 环境变量 ═══');
  console.log(`  SEIKO_LEVEL=${process.env.SEIKO_LEVEL || '(未设置, 默认medium)'}`);
  console.log(`  SEIKO_OFF=${process.env.SEIKO_OFF || '(未设置)'}`);
  console.log(`  SEIKO_PLATFORM=${process.env.SEIKO_PLATFORM || '(未设置, 自动检测)'}`);
}
...
function main() {
  ...
  process.stdin.on('end', () => {
    ...
    // 自动检测平台
    const platform = process.env.SEIKO_PLATFORM || 'claude';
    let output;
    if (platform === 'qoder') {
