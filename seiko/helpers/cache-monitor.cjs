#!/usr/bin/env node
/**
 * cache-monitor.cjs — KV Cache 命中率监控 (Seiko L2 · DeepSeek 专版)
 *
...
const STATS_FILE = path.join(os.tmpdir(), 'seiko-cache-stats.json');
...
function generateReport(stats) {
  ...
  lines.push('       Seiko KV Cache 监控报告');
  lines.push('═'.repeat(50));
  lines.push('');
...
function watchMode() {
  console.log('🔍 Seiko Cache Monitor — Watch 模式');
...
function main() {
  ...
  if (stats.requests === 0) {
    console.log('═══════════════════════════════════');
    console.log('  Seiko KV Cache Monitor');
    console.log('═══════════════════════════════════');
    console.log('');
    console.log('📭 暂无缓存数据。');
    console.log('');
