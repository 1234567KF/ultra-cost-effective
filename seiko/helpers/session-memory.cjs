#!/usr/bin/env node
/**
 * session-memory.cjs — 统一会话压缩记忆索引 (Seiko L6)
 *
 * 无论用 tokenforge 还是 headroom，每次管道压缩都记录到此索引。
 * 切换压缩机时，LLM 可追溯之前压缩过的内容。
 *
 * 存储结构（临时目录 seiko-session-memory.json）：
 * {
 *   sessionId: "sess_xxx",
...
function getSummary() {
  ...
  let summary = `[Seiko会话压缩索引: ${session.records.length}条记录`;
...
function getContextCompressionHint() {
  ...
  let hint = `[SeikoMemory: ${session.records.length}条压缩记录, 共省${(totalSaved/1024).toFixed(1)}KB上下文. `;
  hint += `最近: `;
...
function getAgentSpawnContext() {
  ...
  return {
    summary: summary || `[SeikoMemory: ${session.records.length}条压缩记录]`,
...
function main() {
  ...
  const session = getOrCreateSession();
  console.log('═══════════════════════════════════');
  console.log('  Seiko Session Memory');
  console.log('═══════════════════════════════════');
