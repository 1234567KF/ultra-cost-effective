...
Seiko 上下文拦截器确保在每次 LLM 调用前，上下文已被压缩。此规则在系统提示中持续生效，是三色灯 AOP 拦截体系的第 1 层（规则层）。
...
4. **继承节省**：子 Agent 自然获得已压缩的上下文，无需额外配置 Seiko
...
- Seiko 不控制其他技能的内部 LLM 调用，只控制它们接收到的上下文质量

## 手动检查

```bash
# 查看当前上下文健康
node seiko/helpers/context-interceptor.cjs check

# 生成 LLM 上下文提示
node seiko/helpers/context-interceptor.cjs hint

# Agent spawn 前检查
node seiko/helpers/context-interceptor.cjs pre-agent-spawn
```
