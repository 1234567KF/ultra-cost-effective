#!/usr/bin/env node
/**
 * hook-adapter.cjs — Qoder Hook 适配器 (Seiko)
 *
...
function detectPlatform() {
  // 1. 环境变量优先
  if (process.env.SEIKO_PLATFORM === 'qoder') return 'qoder';
  if (process.env.SEIKO_PLATFORM === 'claude') return 'claude';
...
function unifiedToQoder(result) {
  ...
  return JSON.stringify({
    ...
    reason: result.reason || 'seiko hook adapter'
  });
}
...
function unifiedToClaude(result) {
  ...
  if (result.modifiedCommand) {
    output.hookSpecificOutput = {
      ...
      permissionDecisionReason: result.reason || 'seiko: auto-compress',
...
function adapt(input) {
  ...
  const reason = `seiko: tokenforge ${config.type}/${config.level}`;
