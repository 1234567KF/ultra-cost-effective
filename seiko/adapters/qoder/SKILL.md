...
version: 1.0.0
triggers: seiko, 节能, 省token, 节省, token report, 成本报告
role: infrastructure
scope: global
always-on: true
platform: qoder
...
- 自动检测平台（`SEIKO_PLATFORM=qoder` 或运行时特征）

### 与其他技能的路由
- Seiko 作为 `always-on` 基础设施技能，优先级高于项目技能
- 不影响其他 Qoder 技能的正常加载
