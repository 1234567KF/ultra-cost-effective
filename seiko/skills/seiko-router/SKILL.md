...
triggers: router, 路由, 模型切换, 模型, seiko-router
role: infrastructure
scope: global
always-on: true
...
# Seiko Router · DeepSeek 双模型智能路由 (L7)
...
| 连续 3 次 Pro 失败 | 强制 Flash，锁 10min | 手动 `seiko router reset` |
...
```bash
# 强制使用 Pro
export SEIKO_FORCE_MODEL=pro

# 强制使用 Flash
export SEIKO_FORCE_MODEL=flash

# 恢复自动路由
unset SEIKO_FORCE_MODEL
```
