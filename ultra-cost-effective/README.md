...
重启平台即可，UltraCostEffective 会全自动运行。
...
│   └── ultra-cost-effective-monitor/          L0 全链路监控
├── presets/           ← 三层节能预设
...
安装后 UltraCostEffective 全自动运行，无需干预：
...
| `ultra-cost-effective status` | 显示各层运行状态、缓存命中率、预计节省 |
| `ultra-cost-effective off` | 临时暂停节能（调试用） |
...
**Q: 两个平台怎么共用？**
核心引擎通过 `__dirname` 自动定位，无需修改任何路径。同一份 `ultra-cost-effective/` 目录，两边各自合并对应的 settings 即可。

**Q: 如何卸载？**
删除 `ultra-cost-effective/` 目录，移除 settings 中 ultra-cost-effective 相关配置，重启平台。
